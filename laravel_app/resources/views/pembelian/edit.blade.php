...
@section('title', 'Edit Pembelian — ' . config('app.name'))

@section('content')
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
        <h1 class="h4 mb-0">Edit Pembelian: {{ $pembelian->nomor_pembelian }}</h1>
    ...
    <form action="{{ route('pembelian.update', $pembelian) }}" method="POST" id="pembelian-form">
        @csrf
        @method('PUT')
            ...
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-12 col-md-2">
                        ...
                        <input type="text" id="nomor_pembelian" class="form-control form-control-sm bg-light" value="{{ $pembelian->nomor_pembelian }}" disabled readonly>
                        <div class="form-text small" style="font-size: 0.7rem;">Nomor tidak dapat diubah.</div>
                    </div>

                    <div class="col-12 col-md-2">
                        <label for="tanggal" class="form-label small mb-1">Tanggal Pembelian</label>
                        <input type="date" name="tanggal" id="tanggal" value="{{ old('tanggal', $pembelian->tanggal->format('Y-m-d')) }}" class="form-control form-control-sm" required>
                    </div>
                    ...
                    <div class="col-12 col-md-3">
                        ...
                        <select name="supplier_id" id="supplier_id_select" class="form-select form-select-sm" required>
                            ...
                            @foreach ($suppliers as $sup)
                                <option value="{{ $sup->id }}" @selected(old('supplier_id', $pembelian->supplier_id) == $sup->id)>
                                    {{ $sup->nama_supplier }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-12 col-md-2">
                        ...
                        <select name="metode_pembayaran" id="metode_pembayaran" class="form-select form-select-sm" required>
                            <option value="Cash" @selected(old('metode_pembayaran', $pembelian->metode_pembayaran ?? 'Cash') === 'Cash')>Cash</option>
                            <option value="Transfer Bank" @selected(old('metode_pembayaran', $pembelian->metode_pembayaran) === 'Transfer Bank')>Transfer Bank</option>
                            <option value="Tempo" @selected(old('metode_pembayaran', $pembelian->metode_pembayaran) === 'Tempo')>Tempo</option>
                        </select>
                    </div>

                    <div class="col-12 col-md-3">
                        <label for="keterangan" class="form-label small mb-1">Keterangan</label>
                        <input type="text" name="keterangan" id="keterangan" value="{{ old('keterangan', $pembelian->keterangan) }}" class="form-control form-control-sm" placeholder="No. faktur, catatan, dll.">
                    </div>
                </div>
            </div>
        </div>
