...
class TransaksiSeeder extends Seeder
{
    ...
    {
        // 1. Update seluruh harga_beli (modal) produk agar memiliki margin keuntungan 25% (di bawah harga_jual)
        DB::table('produks')
            ->whereRaw('harga_jual > 0 AND (harga_beli >= harga_jual OR harga_beli <= 0 OR harga_beli IS NULL)')
            ->update([
                'harga_beli' => DB::raw('ROUND(harga_jual * 0.75, -2)')
            ]);
        ...
        // 3. Generasi transaksi harian secara konsisten untuk 2 BULAN TERAKHIR (60 Hari)
        // Memastikan SETIAP HARI memiliki omzet penjualan yang cukup tinggi (Rp 4jt - Rp 10jt) 
        // sehingga Laba Bersih harian & bulanan tidak ada yang minus
        for ($daysAgo = 60; $daysAgo >= 0; $daysAgo--) {
            $baseDate = Carbon::now()->subDays($daysAgo);
            
            // Buat 4 sampai 7 transaksi setiap harinya
            $dailyTransactionCount = rand(4, 7);

            for ($t = 0; $t < $dailyTransactionCount; $t++) {
                $date = (clone $baseDate)->setTime(rand(8, 20), rand(0, 59));
                $metode = $paymentMethods[array_rand($paymentMethods)];

                $transaksi = Transaksi::create([
                    'nama_pelanggan' => rand(0, 1) ? 'Pelanggan Umum' : 'Pelanggan ' . rand(1, 15),
                    'tanggal' => $date,
                    'total' => 0,
                    'bayar' => 0,
                    'kembalian' => 0,
